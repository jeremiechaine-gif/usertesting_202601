/**
 * TanStack Table column definitions for Service Order Book
 * Matches REGLES_METIERS.md section 9.7 exactly
...
export const customFilterFn: FilterFn<ServiceOrderRow> = (row, columnId, filterValue, _addMeta) => {
...
// SO Status indicator
const SOStatusIndicator: React.FC<{ status: string }> = ({ status }) => {
  const statusConfig: Record<string, { color: string; icon: React.ReactNode }> = {
    'completed': { color: '#10B981', icon: <CheckCircle2 className="w-4 h-4" /> },
    'in-progress': { color: '#0EA5E9', icon: <Clock className="w-4 h-4" /> },
...
const EngineeringStatusIndicator: React.FC<{ status: string }> = ({ status }) => {
  const statusConfig: Record<string, { color: string; icon: React.ReactNode }> = {
    'ready': { color: '#10B981', icon: <CheckCircle2 className="w-4 h-4" /> },
    'pending': { color: '#FB8C00', icon: <Clock className="w-4 h-4" /> },
    'blocked': { color: '#F4511E', icon: <XCircle className="w-4 h-4" /> },
  };
...
export const columns: ColumnDef<ServiceOrderRow, any>[] = [
  // Selection checkbox
  ...
  // Group: Service Order
  {
    ...
    columns: [
      columnHelper.accessor('ticketsOnSO', {
        id: 'ticketsOnSO',
        header: 'Tickets on SO',
  ...
  // Group: Status
  {
    ...
    header: 'Status',
    columns: [
      columnHelper.accessor('soStatus', {
        id: 'soStatus',
        header: 'SO Status',
        cell: (info) => <SOStatusIndicator status={info.getValue()} />,
        size: 100,
        filterFn: customFilterFn,
        enableResizing: true,
      }),
      ...
      columnHelper.accessor('engineeringStatus', {
        id: 'engineeringStatus',
        header: 'Engineering Status',
        cell: (info) => <EngineeringStatusIndicator status={info.getValue()} />,
        size: 140,
        filterFn: customFilterFn,
        enableResizing: true,
      }),
  ...
  // Group: Execution
  {
    ...
    columns: [
      ...
      columnHelper.accessor('actualEndDate', {
        id: 'actualEndDate',
